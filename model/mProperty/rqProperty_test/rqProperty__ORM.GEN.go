...
func (p *Property) FromArray(a A.X) *Property { //nolint:dupl false positive
	...
	p.ConstructCompletedDate = X.ToS(a[5])
	p.NumberOfFloors = X.ToS(a[6])
	p.BuildingLamination = X.ToS(a[7])
	p.Address = X.ToS(a[8])
	p.District = X.ToS(a[9])
	p.Note = X.ToS(a[10])
	p.CreatedAt = X.ToI(a[11])
	p.CreatedBy = X.ToU(a[12])
	p.UpdatedAt = X.ToI(a[13])
	p.UpdatedBy = X.ToU(a[14])
	p.DeletedAt = X.ToI(a[15])
	return p
}
