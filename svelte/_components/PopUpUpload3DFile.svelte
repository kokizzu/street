<script>
  ...
  /** @type {Props} */
  let { isSubmitted = $bindable(false), uploadingProgressStr = $bindable(''), OnSubmit = async function(file) {
...
<div class="backdrop {isShow ? 'show' : ''}">
  <div class="form-upload">
    ...
    <div class="buttons">
      ...
      <button class="submit-btn" onclick={() => OnSubmit(file3d)} disabled={!file3d}>
        {#if isSubmitted}
          <span>Uploading....</span>
        {/if}
        {#if !isSubmitted}
