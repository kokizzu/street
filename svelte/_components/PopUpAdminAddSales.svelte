<script>
  ...
  /** @type {Props} */
  let { heading = 'Add Sales', isSubmitted = $bindable(false), OnSubmit = async function(salesObj, propKey, realtorEmail) {
  ...
  let realtorEmail = /** @type {string} */ ($state(''));
  ...
  function submitAddSales() {
    ...
    OnSubmit(salesObj, propertyKey, realtorEmail);
  }
</script>
