        ...
        const getAddress = (latLng) => {
            geocoder.geocode({location: latLng}, (results, status) => {
                if (status === google.maps.GeocoderStatus.OK && results.length > 0) {
                    ...
                    growl.showError('Address not found');
        ...
        searchBox.addListener('places_changed', () => {
            ...
            places.forEach((place) => {
                if (!place.geometry || !place.geometry.location) {
                    growl.showWarning('Returned place contains no geometry');
                    return;
                }
                const icon = {
                    url: place.icon,
    ...
    const handleNextLocation = {
        'LOC_ADDR': async () => {
            ...
            if (property.city === '' || property.street1 === '' || property.floors === 0) {
                growl.showWarning('Please fill required form');
                return;
            }
            for (let i = 0; i < countries.length; i++) {
                if (countries[i].iso_2 === property.countryCode) {
        ...
        'LOC_MAP': () => {
            if (property.formattedAddress === '') {
                growl.showWarning('Please mark location on map');
                return;
            }
            modeLocationCount += 1;
            modeLocation = modeLocationLists[modeLocationCount].mode;
    ...
    const handleNextInfo = {
        ...
        'INFO_PRICE': () => {
            if (property.lastPrice === 0 || property.agencyFee >= 99) {
                growl.showWarning('Price cannot be 0');
                return
            }
            nextPage();
        },
        ...
        if (file) {
            ...
            ajax.addEventListener('load', function(event) {
                ...
                if (ajax.status === 200) {
                    ...
                    growl.showSuccess('Image uploaded');
                } else if (ajax.status === 413) {
                    growl.showError('Image too large');
                } else {
                    growl.showError(`Error: ${ajax.status}  ${ajax.statusText}`);
                }
            });
            ajax.addEventListener('error', function(event) {
                growl.showError('Network error');
            });
            ajax.addEventListener('abort', function(event) {
                growl.showError('Upload aborted');
            }, false);
