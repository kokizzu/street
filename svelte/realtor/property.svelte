    ...
    <div class="realtor_step_progress_bar">
      <div class="step_wrapper">
        <div class={currentPage >= 0 ? 'step_item completed' : 'step_item active'}>
          <button on:click={() => progressDotHandler(0)} />
          <p>Location</p>
        </div>
        <div
          class={currentPage === 1
            ? 'step_item active'
            : 'step_item' && currentPage > 1
            ? 'step_item completed'
            : 'step_item'}
        >
          <button on:click={() => progressDotHandler(1)} />
          <p>Info</p>
        </div>
        <div
          class={currentPage === 2
            ? 'step_item active'
            : 'step_item' && currentPage > 2
            ? 'step_item completed'
            : 'step_item'}
        >
          <button on:click={() => progressDotHandler(2)} />
          <p>Floor</p>
        </div>
        <div class={currentPage === 3 ? 'step_item active' : 'step_item'}>
          <button on:click={() => progressDotHandler(3)} />
          <p>Preview</p>
        </div>
      </div>
    </div>
      ...
      <div class="realtor_subpage_container">
        ...
        <section class="info" id="subpage_2" bind:this={cards[1]}>
          <div class="main_info">
              ...
              <div class="upload_house_photo">
                <div class="image_preview_container">
                  <label class="image_upload_button" for="upload_image">
                    ...
                    {:else}
                      <progress value={uploadHousePercent} max="100" />
            ...
            {#if mode === FEATURE_OR_FACILITY}
              ...
              <div class="facility_section">
                <textarea
                  bind:value={facility}
                  rows="10"
                  placeholder="Type the facility in the property."
                  name="facility"
                  id="facility"
                />
              </div>
            ...
            {#if mode === DESCRIPTION_PROPERTY}
              ...
              <div class="description_of_property">
                <textarea
                  bind:value={description_of_property}
                  rows="20"
                  placeholder="Writing a description can help potential buyers become more interested in your property."
                  name="description"
                  id="description"
                />
              </div>
          ...
          <div class="next_skip_button">
            {#if modeSkippable === true}
              <button class="skip_button" on:click|preventDefault={houseInfoSkip}> SKIP </button>
            {/if}
            <button
        ...
        <section class="floor" id="subpage_3" bind:this={cards[2]}>
          ...
          >
            <button
              disabled={room_type === ''}
              class="add_room_button"
              on:click={() => handlerAddRoom(floor_index_to_edit)}>Add</button
            >
          </AddRoomDialog>
          <div class="floor_main_content">
            ...
            <div class="floor_header">
              {#if floor_edit_mode === true}
                <h3>
                  Edit {floor_lists[floor_index_to_edit].type}
                  {floor_lists[floor_index_to_edit].type === 'basement'
                    ? ''
                    : `#${floor_lists[floor_index_to_edit].floor}`}
                </h3>
              {:else}
                <h2>Floors</h2>
            ...
            {#if floor_edit_mode === true}
              <div class="edit_floor_container">
                {#if imgFlrPlanUploading === true}
                  <div class="uploading">
                    <progress value={uploadFlrPlanPercent} max="100" />
                    <p>{uploadFlrPlanStatus}</p>
                  </div>
                {:else if floor_lists[floor_index_to_edit].planImageUrl === ''}
...
<style>
  ...
  .floor .floor_items_container .floor_item .beds,
  .floor_item .baths {
    display: flex;
    flex-direction: column;
    text-align: center;
  ...
  .floor .floor_items_container .floor_item .beds b,
  .floor_item .baths b {
    font-size: 25px;
  }

  .floor .floor_items_container .floor_item .beds p,
  .floor_item .baths p {
    margin: 10px 0 0 0;
  }
