<script>
  ...
  onMount( () => {
    ...
    if( Object.keys( property ).length===0 ) {
      ...
      property = {
        countryCode: country,
        city: '', // new, but already exists on prop table
        address: '',
        district: '',
        street1: '', // new, as payload will be .address separated by "\n"
        street2: '', // new, combined to address
        numberOfFloors: 1,
        formattedAddress: '',
        lat: lat,
        lng: lng,
        coord: [lat, lng],
        altitude: 0, // new, added
        ...
        bedroom: 0,
        bathroom: 0,
        livingroom: 0, // new, added
        sizeM2: 0,
        purpose: 'sell',
        agencyFeePercent: 0,
        parking: '0', // new, added
        depositFee: 0, // new, added
        minimumDurationYear: 0, // new, added
        otherFees: [], // new, added
        lastPrice: 0,
      ...
      property.floorList = property.floorList || []; // TODO: deprecate
      property.images = property.images || [];
      let addr = property.address.split( '\n' );
      property.street1 = addr[ 0 ] || '';
      property.street2 = addr[ 1 ] || '';
    }
  } );
  ...
  $: {
    ...
    payload.id = id;
    payload.address = property.street1 + '\n' + (property.street2 || '');
    payload.parking = +property.parking || 0;
    payload.minimumDurationYear = +property.minimumDurationYear || 0;
    payload.otherFees = property.otherFees || [];
    payload.altitude = +property.altitude || 0;
    payload.images = property.images || [];
    payload.coord = [property.lat, property.lng];
    payload.lastPrice = ''+(property.lastPrice || 0);
    payload.sizeM2 = ''+(property.sizeM2 || 0);
    payload.numberOfFloors = ''+(property.numberOfFloors || 0);
  }
  ...
  let modeLocationCount = 0, countryName = 'Country', countryIso2 = user.countryCode;
    ...
    searchBox.addListener( 'places_changed', () => {
      ...
      for( let i = 0; i<places[ 0 ].address_components.length; i++ ) {
        if( places[ 0 ].address_components[ i ].types.indexOf( 'country' )!== -1 ) {
          property.countryCode = places[ 0 ].address_components[ i ].short_name || '';
  ...
  const handleNextLocation = {
    'LOC_ADDR': async () => {
      ...
      if( property.city==='' || property.street1==='' || property.numberOfFloors===0 ) {
        useGrowl( 'error', 'Please fill required form' );
        return;
      }
      countries.forEach( ( c ) => {
        if( c.iso_2===property.countryCode ) {
  ...
  let otherFeesObj = ['', 0]; // name, fee, using array since tarantool quite annoying to store object
  let addOtherFeesDialog = AddOtherFeesDialog;
  let houseTypeLists = [
    'house', 'land', 'apartment', 'townhouse', 'condo', 'villa', 'factory', 'parking', 'other',
  ];
  
  function addotherFees() {
    property.otherFees = [...property.otherFees, otherFeesObj];
    otherFeesObj = ['', 0];
    addOtherFeesDialog.hideModal();
  }
    ...
    <div class='content'>
      <div class='realtor_subpage_container'>
              ...
              <div class='address'>
                ...
                <div class='row'>
                  <div class='input_box'>
                    ...
                    <input id='floors' type='number' placeholder='10' min='1' bind:value={property.numberOfFloors} />
                  </div>
                </div>
              </div>
            {/if}
            ...
            {#if modeLocation===LOC_STREETVIEW}
              <div class='location_streetview'>
                ...
                <div class='streetview_container'>
                  <div class='input_box'>
                    <label for='altitude'>How many meter from floor where you want to put your signage?</label>
        ...
        <section bind:this={cards[1]} class='info' id='subpage_2'>
          <AddOtherFeesDialog
            bind:fee={otherFeesObj[1]}
            bind:name={otherFeesObj[0]}
            bind:this={addOtherFeesDialog}
          >
            <button class='add_fee_btn' on:click={addotherFees}>
              Add
            </button>
          </AddOtherFeesDialog>
              ...
              <div class='price'>
                ...
                {#if property.purpose==='rent'}
                  ...
                  <div class='other_fee'>
                    <header>
                      <h4>Other Fee</h4>
                      <button class='add_fee' on:click={addOtherFeesDialog.showModal()}>
                        Add
                      </button>
                    </header>
                    <div class='other_fee_lists'>
                      {#if property.otherFees && property.otherFees.length}
                        {#each property.otherFees as otherFees}
                          <div class='fee'>
                            <span>{otherFees.name}</span>
                            <b>{formatPrice( otherFees.fee, countryCurrency )}/mo</b>
                          </div>
                        {/each}
                      {:else}
                        <div class='fee'>
                ...
                <div class='preview_details'>
                  ...
                  <div class='other_details'>
                    {#if property.purpose==='rent'}
                      ...
                      {#if infoObj.otherFees && infoObj.otherFees.length}
                        <div class='details'>
                          <h5>Other Fee</h5>
                          {#each infoObj.otherFees as otherFees}
                            <div class='item'>
                              <span>{otherFees.name}</span>
                              <b>{formatPrice( otherFees.fee, countryCurrency )}/mo</b>
                            </div>
          ...
          {#if isPropertySubmitted===true}
            ...
            <div class='property_submitted_container'>
              ...
              <div class='actions'>
                ...
                <button class='see' on:click={() => {window.location.href = `/realtor/ownedProperty/${property.id}`}}>See the property</button>
              </div>
            </div>
          {/if}
        </section>
